...
namespace ExaminationSystem.Controllers
{
    public class QuestionController : Controller
    {
        ...
        /// <param name="tagsId"></param>
        /// <returns></returns>
        public string AddJudgment(string title, bool isTrue, int score, int[] tagsId)
        {
            int code;
            string message;
        ...
        /// <param name="pageIndex"></param>
        /// <returns></returns>
        public string GetSingleByTagId(int tagId, int pageIndex = 1)
        {
            int code;
            string message;
            try
            {
                var questions = from e in db.ES_Exercise
                                where e.EsType == "单选题" && e.IsDel == false
                                join sq in db.ES_SelectQuestion on e.EsSubExerciseId equals sq.SQId
                                join te in db.ES_Tag_Exercise on e.EsId equals te.EsId
                                where te.TagId == tagId
                                select new
                                {
                                    e.EsId,
                                    sq.SQTitle,
        ...
        /// <param name="pageIndex"></param>
        /// <returns></returns>
        public string GetMultipleByTagId(int tagId, int pageIndex = 1)
        {
            int code;
            string message;
            try
        ...
        public string GetJudgment(int pageIndex = 1)
        {
            ...
            try
            {
                ...
                foreach (var question in questions)
                {
                    ...
                    bool isTrue = question.JQIsTrue;
                    int score = question.JQScore;

                    questionList.Add(new { id, title, isTrue, score });
                }
        ...
        /// <param name="pageIndex"></param>
        /// <returns></returns>
        public string GetJudgmentByTagId(int tagId, int pageIndex = 1)
        {
            int code;
            string message;
            try
        ...
        /// <param name="pageIndex"></param>
        /// <returns></returns>
        public string GetFillByTagId(int tagId, int pageIndex = 1)
        {
            int code;
            string message;
            try
